...
func (d *driver) populateNetworks() error {
	...
	if err != nil && err != datastore.ErrKeyNotFound && err != boltdb.ErrBoltBucketNotFound {
		return fmt.Errorf("failed to get ovs network configurations from store: %v", err)
	}
	...
	for _, kvo := range kvol {
		...
		if err = d.createNetwork(ncfg); err != nil {
			logrus.Warnf("could not create ovs network for id %s ovs name %s while booting up from persistent state", ncfg.ID, ncfg.BridgeName)
		}
	}
...
func (d *driver) storeUpdate(kvObject datastore.KVObject) error {
	if d.store == nil {
		logrus.Warnf("ovs store not initialized. kv object %s is not added to the store", datastore.Key(kvObject.Key()...))
		return nil
	}

	if err := d.store.PutObjectAtomic(kvObject); err != nil {
		return fmt.Errorf("failed to update ovs store for object type %T: %v", kvObject, err)
	}
...
func (d *driver) storeDelete(kvObject datastore.KVObject) error {
	if d.store == nil {
		logrus.Debugf("ovs store not initialized. kv object %s is not deleted from store", datastore.Key(kvObject.Key()...))
		return nil
	}
